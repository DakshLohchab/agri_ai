...
\begin{itemize}[leftmargin=*, label=\textcolor{brandblue}{\textnormal{\faLeaf}}, itemsep=2pt]
    \item \textbf{System Drop Rate:} Approximately 800 queries drop per month due to rural network timeouts.
    \item \textbf{Manual Recovery Cost:} Support agents must manually call back farmers to resolve dropped queries.
    \item \textbf{Time \& Labor:} Each manual callback/investigation takes an average of 15 minutes.
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash\bfseries}p{4cm} X >{\raggedright\arraybackslash\bfseries}p{3.2cm}}
\toprule
Metric & \normalfont\textbf{Calculation Logic} & Impact \\
\midrule
Total Manual Time Avoided & 800 drops $\times$ 80\% handled locally $\times$ 15 mins per resolution & 160 Hours / mo \\
\addlinespace
Labor Cost Reduced & 160 hours saved $\times$ \textnormal{\faInr} 150 per hour & \textnormal{\faInr} 24,000 / mo \\
\addlinespace
API Costs Avoided & 640 fallback queries rerouted from cloud LLM & Negligible \\
\bottomrule
\end{tabularx}
...
Metric & \normalfont\textbf{Calculation Logic} & Impact \\
\midrule
Churn Events Prevented & 640 local resolutions $\times$ 20\% risk of churn if left unanswered & 128 Users Retained \\
\addlinespace
Monthly Revenue Protected & 128 retained users $\times$ \textnormal{\faInr} 50 ARPU & \textnormal{\faInr} 6,400 / mo \\
\bottomrule
\end{tabularx}
